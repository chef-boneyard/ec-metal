...
describe 'private-chef::ec2' do

  context 'When all attributes are default, on an unspecified platform' do

    let(:chef_run) do
      runner = ChefSpec::ServerRunner.new do |node|
        node.automatic['filesystem'] = { '/dev/sda1' => { 'mount' => '/' } }
    ...
    before do
      stub_command("grep /mnt /proc/mounts").and_return('/mnt')
      stub_command("grep /mnt /etc/fstab").and_return('/mnt')
    end
